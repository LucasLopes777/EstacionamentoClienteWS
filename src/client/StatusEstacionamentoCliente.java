...
public class StatusEstacionamentoCliente extends JFrame {
	...
	public StatusEstacionamentoCliente() throws MalformedURLException {
		...
		btnAtualizar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					statusService = cerragarServico();
				} catch (MalformedURLException e1) {
					e1.printStackTrace();
				}
				
				StatusBean bean = statusService.getStatus(); // Chamada do WS
				carregarLabels(bean);
	...
	protected StatusEstacionamentoService cerragarServico() throws MalformedURLException {
		StatusEstacionamentoService StatusEstacionamentoService = null;
		
		//Definir a URL de acesso ao serviço
		URL url = new URL("http://127.0.0.1:8888/status?WSDL");
		
		//Qname (Qualified Name) é a url ao contário e terminado com "/" impacta.ead.jp.estacionamento.integracao
		QName qname = new QName("http://integracao.estacionamento.jp.ead.impacta/",
				
				/*
				 * Esse 2º parâmetro é o nome da classe de implementação da interface ou
				 * seja, o StatusEstacionamentoServiceImpl do WebServivce e deve se acrescentar o
				 * Service no final ficando 
				 *  StatusEstacionamentoServiceImplService
				 */
				"StatusEstacionamentoImplService");
		
		//Criação de um proxy de comunicação com o serviço
		Service service = Service.create(url, qname);
		
		StatusEstacionamentoService = service.getPort(StatusEstacionamentoService.class);
		
		return StatusEstacionamentoService;
	}
}
